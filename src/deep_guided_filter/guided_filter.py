...
class FastGuidedFilter(nn.Module):
    def __init__(self, r, c_lr, c_hr, eps): #lower epsilon leads to higher guidence
        ...
        self.convA = nn.Conv2d(c_lr, c_hr, kernel_size=1, stride=1)
        self.convb = nn.Conv2d(c_lr, c_hr, kernel_size=1, stride=1)
    ...
    def forward(self, lr_x, lr_y, hr_x):
        ...
        # assert c_lrx == c_hrx and (c_lrx == 1 or c_lrx == c_lry) c_lrx and c_hrx don't match when used in unet architecture 
        assert self.c_hr == c_hrx and self.c_lr == c_lrx and c_lrx == c_lry # instead
        ...
        ## reduce channels
        mean_A = self.convA(mean_A)
        mean_b = self.convb(mean_b)
